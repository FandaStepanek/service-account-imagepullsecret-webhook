...
def copy_secret_to_namespace(namespace):
    ...
    if response.status_code != 200:
        print(f" ERROR: Failed to get `{DOCKERHUB_SECRET_NAME}` from `{DEFAULT_NAMESPACE}`")
        return False
    ...
    if response.status_code == 201:
        print(f" Copied `{DOCKERHUB_SECRET_NAME}` to `{namespace}`")
        return True
    else:
        print(f" ERROR: Failed to create `{DOCKERHUB_SECRET_NAME}` in `{namespace}`: {response.text}")
        return False
